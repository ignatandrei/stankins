...
namespace SenderHTML
{
    public class Sender_HTMLRelationViz : ISend
    {
        ...
        public Sender_HTMLRelationViz(string folderNameWithRootAndRelation, string label, string outputFileName)

        {
            this.Name = "sender html viz";
            FolderNameWithRootAndRelation = folderNameWithRootAndRelation;
            OutputFileName = outputFileName;
            Label = label;
            VizUrl = "https://github.com/mdaines/viz.js/releases/download/v1.8.0/viz.js";
        }

        public string FolderNameWithRootAndRelation { get; set; }
        ...
        public async Task Send()
        {
            ...
            string fileNameRoot = Path.Combine(FolderNameWithRootAndRelation, "root.cshtml");

            if (!File.Exists(fileNameRoot))
            {
                throw new FileNotFoundException($"root {fileNameRoot} must exists", fileNameRoot);
            }
            var mediaTransform = new MediaTransformRazor(fileNameRoot);
